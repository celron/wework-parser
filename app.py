...
def readGeoGroup(group):
    response = urllib2.urlopen('https://api-proxy.wework.com/locations/api/v1/geogroupings/'+group)
    a = response.read()
    return json.loads(a)


def readBuilding(building):
    response = urllib2.urlopen('https://api-proxy.wework.com/locations/api/v2/buildings/'+building)
    a = response.read()
    return json.loads(a)
